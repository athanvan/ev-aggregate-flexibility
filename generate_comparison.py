...
mean_icnn = np.mean(ml_mean)
se_icnn = np.std(ml_mean) / (9 ** 0.5)

mean_sp = np.mean(sp_mean)
se_sp = np.std(sp_mean) / (9 ** 0.5)

mean_ga = np.mean(ga_mean)
se_ga = np.std(ga_mean) / (9 ** 0.5)
...
plt.ylabel("Peak power gap (kW)", fontsize=15)
plt.xticks([1, 2, 3], ["ICNN", "struc.-pres.", "gen. affine"], fontsize=15)

ax = plt.gca()
y_min, y_max = ax.get_ylim()
ax.set_ylim(y_min, y_max + 0.15 * (y_max - y_min)) 
text_y = y_max + 0.05 * (y_max - y_min)
ax.text(1, text_y, f"{mean_icnn:.2f} $\\pm$ {se_icnn:.2f}", ha='center', fontsize=12, fontweight='bold')
ax.text(2, text_y, f"{mean_sp:.2f} $\\pm$ {se_sp:.2f}", ha='center', fontsize=12)
ax.text(3, text_y, f"{mean_ga:.2f} $\\pm$ {se_ga:.2f}", ha='center', fontsize=12)

plt.savefig("ppm_boxplot.pdf", pad_inches=0, bbox_inches='tight')

print(f"Mean ICNN Gap: {mean_icnn}")
print(f"ICNN Standard Error: {se_icnn}")
print(f"Mean General Affine Gap: {mean_ga}")
print(f"General Affine Standard Error: {se_ga}")
print(f"Mean Structure Preserving Gap: {mean_sp}")
print(f"Structure Preserving Standard Error: {se_sp}")