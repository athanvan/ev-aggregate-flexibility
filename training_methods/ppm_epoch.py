...
class PPM_Epoch():
    def __init__(self, load_train, load_val, translation,
        ...
        Args: 
            ...
            model_checkpoint_path: file path for saving model weights
        """
    ...
    def ppm_epoch(self, use_cvxpylayer):
        """
        ...
        start = time.time()
        if use_cvxpylayer:
            sol = self.contain_class.solve_cvxpylayer_icnn()
            ratio = sol[0]
            return_vals["ratio"] = torch.clone(sol[0]).detach().cpu().numpy()
        ...
        with torch.no_grad():
            ...
            ratio = torch.as_tensor(1).float().to(self.device)
        
        return_vals["time"] = time.time() - start 
        load_val = torch.as_tensor(self.load_val).to(self.device).float()
        ppm_val_loss = self.ppm_loss.ppm_evaluate(load_val, layer = False, ratio = torch.clone(ratio))
        ...
        sol = self.ppm_loss.ppm_evaluate(load_train, layer = True, ratio = ratio)
        # recompute task loss with the solution from cvxpylayer 
        ppm_training_loss = 0
        for idx in range(load_train.shape[1]):
            ppm_training_loss += torch.linalg.norm(ratio * sol[0][:, idx] + translation + load_train[:, idx], ord = float("inf"))
