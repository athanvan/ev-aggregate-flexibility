...
class PPM_Loss:
    def __init__(self, translation,
                 ...
                 val_batch_size, if_verbose, solver):
        """
        Args: 
    ...
    def ppm_init(self, batch_size, if_layer):
        ...
        """
        u = cp.Variable((self.T, batch_size), name = "u")
        var_list  = [u]
    ...
    def create_ppm_constraints(self, u, l, param_list, var_list, 
        ...
        for idx in range(batch_size):
            obj += cp.norm_inf((ratio * u[:, idx]) + cp.Constant(self.translation) + l[:, idx])
    ...
    def ppm_evaluate(self, l, layer = True, return_u = False, ratio = torch.as_tensor(1)):
        """
        ...
        if layer: 
            try:
                sol = self.ppm_layer(
                    *([l, pmin, pmax] + fc_weights + rl_weights + rl_biases + [ratio]), solver_args = self.keywords)
            ...
            for rl_b in rl_biases:
                ...
                idx += 1
            self.ppm_val_params[idx].value = ratio.cpu().detach().numpy()
            idx += 1
            self.ppm_problem_val.solve(solver = self.solver, **self.keywords)
            if return_u:
                return self.ppm_problem_val.var_dict["u"].value 
